...
<script lang="ts">
  ...
  import { Egateway, EgatewayType, IgatewayTranscation, IthriftTranscation } from "../../api/transcation/gateway";
    ...
    const paystack = new win.PaystackPop();
      paystack.newTransaction({
        ...
        onSuccess: async (transaction) => {
          ...
          handleNotification('submitting transcations', window, 'info','loading...');
          try {
           const resp = await axios.post('api/thrift/daily/transcation',{trans, trnasList});
...
<div class="container-fluid pb-5 ">
  ...
  <ul class="items-list mt-3">
    ...
    <li>
      ...
      <span class="second-action mif-checkmark fg-green"></span>
      {:else if transcation.resolved == 0}
      <span class="second-action mif-pause fg-blue"></span>
      {:else}
      <span class="second-action mif-cancel fg-red"></span>
      {/if}
  </li>
    {/each}
