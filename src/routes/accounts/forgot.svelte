<script lang="ts">
    ...
    const submit = async () => {
      try {
        disabled = true;
        const resp = await axios.put("api/accounts", user);
        console.log(resp);
  ...
  <svelte:head>
    <title>SafeSave :: Forget Page</title>
  </svelte:head>
    ...
    <div class="login-page pt-5">
      <div class="form">
        <form on:submit|preventDefault={submit} class="login-form">
          ...
          <p class="text-center mb-4">Enter your safesave</p>
          <input
          ...
          <button {disabled} type="submit"
            >{disabled ? "checking..." : "forget password"}</button
          >
          <p class="message">
